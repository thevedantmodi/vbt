...
export default function EditorialScreen({ transactions = [], dark = false, accent = '#4F63D2', budgets = {}, onSetBudget, onRefreshTransactions, onToggleDark, linked, serverUp, loading, demo, onLink, onSync, onUnlink }: Props) {
  ...
  return (
    <div style={{ position: 'relative', height: '100%', background: T.bg, color: T.text, fontFamily: '-apple-system, system-ui, sans-serif', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <Sheet cat={oc} T={T} dark={dark} monthLabel={MONTH_NAMES[d.month]} year={d.year} isCurrent={d.isCurrent} onClose={() => setOpenId(null)} onSetBudget={onSetBudget ?? (() => {})} onRefreshTransactions={onRefreshTransactions ?? (() => {})} />
    </div>
  );
}
