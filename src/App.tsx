...
export default function App() {
          ...
          {!linked && (
            <button
              ...
              style={{
                ...btn,
                background: dark ? "#111" : "#fff",
                opacity: serverUp ? 1 : 0.4,
              }}
            >
              {serverUp ? "+ Connect bank" : "Server offline"}
            </button>
          )}
          {linked && (
            <>
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={() => setDark((v) => !v)} style={btn}>
          {dark ? "☀︎ Light" : "☾ Dark"}
        </button>
        <button onClick={() => setDesktop(true)} style={btn}>
          ⬜ Desktop
        </button>
        {!linked && (
          <button
            onClick={startLink}
            disabled={!serverUp}
            style={{ ...btn, opacity: serverUp ? 1 : 0.4 }}
          >
            {serverUp ? "+ Connect a bank (Plaid)" : "Server offline"}
          </button>
        )}
        {linked && (
          <>
